...
APP = Flask(__name__, template_folder='C:/BaseDados/tra/mytemplates')
...
def index():
    ...
    SELECT
      ...
      (SELECT COUNT(*) FROM Cerimonies) as n_cerimonies
    ''').fetchone()
    logging.info(stats)
...
def get_nominee(nominee_id):
    ...
    nominations = mydb.execute('''
        SELECT Categories.name AS category, Nominations.winner, Cerimonies.year
        FROM NominationEntities
        JOIN Nominations ON NominationEntities.nomination = Nominations.id
        JOIN Categories ON Nominations.category = Categories.id
        JOIN Cerimonies ON Nominations.cerimony = Cerimonies.cerimony
        WHERE NominationEntities.nominee = ?
        ORDER BY Cerimonies.year
    ''', [nominee_id]).fetchall()
...
def search():
    ...
    ceremonies = mydb.execute('''
        SELECT cerimony, year FROM Cerimonies WHERE year LIKE ?
    ''', ['%' + search_term + '%']).fetchall()
...
def list_ceremonies():
    ...
    ceremonies = mydb.execute('''
        SELECT cerimony, year FROM Cerimonies
...
def get_ceremony(ceremony_id):
    ...
    ceremony = mydb.execute('''
        SELECT cerimony, year FROM Cerimonies WHERE cerimony = ?
